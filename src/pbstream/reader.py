...
class PBstream_Reader:
    ...
    def info(file_name: Union[str, Path]) -> None:
        ...
        for field_name, counter in loaded_data.items():
            print(
                "Field:"
                f' {field_name: <{max_key_length}}\t{counter: >{max_charlength_items}} {"entries" if counter != 1 else "entry"}'
            )
    ...
    def _read_header(self) -> None:
        ...
        assert (
            self.initial_offset == 0
        ), "no read operation should occur prior to _read_header"
        # check the first 8 bit
        data = self.filehandle.read(8)
        assert self.version_magic == self._readsize(
            data
        ), "version magic does not match the file start"
